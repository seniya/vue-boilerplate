<template>
  <a-card hoverable style="width: 100%" :loading="isLoadingArticleRead">
    <a-form
      :form="form"
      :label-col="{ span: 5 }"
      :wrapper-col="{ span: 12 }"
      ...
      <a-form-item :wrapper-col="{ span: 12, offset: 5 }">
        <a-button
          type="primary"
          html-type="submit"
          :loading="isLoadingArticleUpdate">
          Submit
        </a-button>
      </a-form-item>
    </a-form>
...
export default {
  props: {
    ...
    isLoadingArticleRead: {
      type: Boolean,
      required: true
    },
    isLoadingArticleUpdate: {
      type: Boolean,
      required: true
    },
    prepareData_: {
  ...
  watch: {
    item: function () {
      if (!this.isLoadingArticleRead) this.cdSetFieldsValueWithAntd()
    }
  },
